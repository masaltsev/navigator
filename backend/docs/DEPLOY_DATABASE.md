...
## Тесты не на продовой БД

**Запрещено** запускать на продовой базе тесты, которые создают записи в БД (Feature-тесты вроде `SourceTest`, `ImportTest` и др.). Они создают тестовые организации («Source Test Org», «Lookup by Source ID Org») и источники с URL на example.com; эти записи попадают в выборку due и в дампы. На проде тесты либо не запускают против продовой БД, либо для тестов используется отдельная тестовая БД. См. [scripts/clean_example_com_sources.md](scripts/clean_example_com_sources.md).
...
**Cron для due sources (плановый обход):** инструкции и чеклист для проды — [harvester-dispatch-due.md](./harvester-dispatch-due.md). Кратко: в crontab раз в минуту `* * * * * cd /path/to/backend && php artisan schedule:run`; Laravel по расписанию (02:00) запускает `harvest:dispatch-due --limit=500`.
